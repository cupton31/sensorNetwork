public class FordFulkerson {
    ...
    public FordFulkerson(NetworkFlow G, int s, int t) {
        if (s == t) {
    ...
    private boolean hasAugmentingPath(NetworkFlow G, int s, int t) {
    ...
    private double excess(NetworkFlow G, int v) {
        double excess = 0.0;
        for (FlowEdge e : G.adj(v)) {
    ...
    // return excess flow at vertex v
    private boolean isFeasible(NetworkFlow G, int s, int t) {
        ...
        /*for (int v = 0; v < G.V(); v++) {
            for (FlowEdge e : G.adj(v)) {
                if (e.flow() < -EPSILON || e.flow() > e.capacity() + EPSILON) {
                    System.err.println("Edge does not satisfy capacity constraints: " + e + "   flow = "+e.flow+"   capacity = "+e.capacity);
                    return false;
                }
            }
        }*/
    ...
    // check optimality conditions
    private boolean check(NetworkFlow G, int s, int t) {
